...
from regression.src.utils.inference import *
...
def main(args):
    ...
    run_path = os.path.join(f"../models", args.task, "pretrained")

    # Prepare the model, get the task and data samplers, and save the parameters required later
    model, conf, task_sampler, covariate_sampler, params = prepare_model(run_path, args.batch_size, device)
    ...
    experiment_dir = f"./LTV_models/{args.task}/{args.act_fn}/seq_len_{args.seq_len}"

    if not os.path.exists(experiment_dir):
        os.makedirs(experiment_dir)

    task_long = task_sampler()
    xs_long = covariate_sampler.sample_xs(b_size=args.batch_size, n_points=args.seq_len).to(device)
    ys_long = task_long.evaluate(xs_long).to(device)

    n_samples = int(args.n_batches * args.batch_size)
    ...
    if os.path.exists(os.path.join(experiment_dir, 'data_samples.pt')) and os.path.exists(
        ...
        long_data = torch.zeros(n_samples, args.seq_len, n_dims)
        long_label = torch.zeros(n_samples, args.seq_len, dtype=model._backbone.dtype)

        curr_idx, batch_i = 0, 0

        while curr_idx < n_samples:
            data, labels, xs_long, ys_long = data_sampler(model, conf, task_sampler, covariate_sampler, args.seq_len,
                                                          args.batch_size, n_layers, n_heads, head_dim, resid_dim, device)
            labels = labels[:, -1].unsqueeze(dim=1)

            space_left = n_samples - curr_idx
            end_idx = curr_idx + min(args.batch_size, space_left)
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=args.sgd_batch_size, shuffle=True)
    val_dataloader = DataLoader(val_dataset, batch_size=args.sgd_batch_size, shuffle=True)

    ltv_layer = LearnableTaskVector(n_layers, n_heads).to(device)
    loss_fn = F.huber_loss
    optimizer = optim.Adam(ltv_layer.parameters(), lr=args.lr)
    ...
    for epoch_i in range(args.n_epochs):
        ...
        print(
            f'Epoch [{epoch_i + 1}/{args.n_epochs}] - training loss: {average_training_loss:.4f}, validation loss: {average_val_loss:.4f}')

        # Save model if validation loss is the lowest
        if average_val_loss < lowest_val_loss:
            lowest_val_loss = average_val_loss
            torch.save(ltv_layer.state_dict(), os.path.join(experiment_dir, f'ltv_layer_{args.seq_len}.pth'))
            print('Checkpoint saved with lowest validation loss')
        else:
            no_improvement_count += 1

        # Early stopping check
        if no_improvement_count >= args.early_stoppage_tolerance:
            print(
